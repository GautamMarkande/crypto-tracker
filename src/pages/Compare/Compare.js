...
function Compare() {
   ...
   const [isLoading,setisLoading] = useState(true)
    ...
    const handleDaysChange = async (event) => {
        ...
        if(Crypto1&&Crypto2){
            const prices1 = await GetCoinPrice(Crypto1,event.target.value,"prices");
            const prices2 = await GetCoinPrice(Crypto2,event.target.value,"prices");
    ...
    return (
        <div className='ComparePageContainer'>
            <div className="selectBoxesContainer">
                {Allcoins&& <SelectCoin Allcoins={Allcoins}  handleChangeCrypto={handleChangeCrypto}  Crypto1={Crypto1} Crypto2={Crypto2}/>}
            ...
            {/* {ChartData&&<LineChart chartData={ChartData} PriceType={"prices"} multiAxis={true}/>} */}
